...
**2026-08-18 更新**:另外用 4 個新 seed(2-5,各自重新隨機切分+重新訓練
35 epoch)驗證這個洩漏穩不穩定——結果 **5 個 seed 全部都是 100.00%**,
一分不差。這其實是更強的證據:不是「運氣好抽到某一組特別容易的切分」,
是這個資料集本身的連拍重複問題太嚴重,不管怎麼隨機切都躲不掉同一片葉子
的照片同時出現在 train 跟 test 裡。
